...
export class AuthService {
  ...
  async changePassword(authRequest: AuthRequest): Promise<object> {
    ...
    const newUser = await firebase
      .auth()
      .updateUser(uid, { password: authRequest.password });
    return newUser;
  }
  ...
  private async firebaseRequest(
    path: string,
    data: object,
  ): Promise<AxiosResponse<any>> {
    return await this.httpService
      .post(
        `https://identitytoolkit.googleapis.com/v1/accounts:${path}?key=${process.env.FIREBASE_API_KEY}`,
        data,
      )
      .toPromise();
  }
